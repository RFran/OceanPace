 ...
 # -------DEF FUNCTIONS------------------------
    ...
    def execution(ser, commande):
        ...
        try:
            ...
            print('? =',
                  ser.inWaiting())  # Look into the input buffer, if it's not empty it's mean that the device received our command
            out = ''
            liste = []
            for i in range(2):
                if ser.inWaiting() != 0:
...
class PACE5000Window(QMainWindow, G_window.Ui_MainWindow):
    def __init__(self):
        QMainWindow.__init__(self)
        G_window.__init__(self)
        self.setupUi(self)
    ...
    def applyparam(self):  # This function allow us to apply new parameters
        ...
        if float(set_p) > max_setpoint:
            choice = QtGui.QMessageBox.question(self, 'Control Mode',
                                                "Are you sure to apply a set point higher than 80?",
